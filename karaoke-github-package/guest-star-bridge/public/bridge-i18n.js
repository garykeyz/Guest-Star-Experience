const PHRASES = [
  ...
  ["Completed songs and the active Player queue cover the activity duration.", "Las canciones completadas y la fila activa del Player cubren la duración de la actividad."],
  ["Close Requests Now", "Cerrar solicitudes ahora"], ["The VirtualDJ queue is empty.", "La cola de VirtualDJ está vacía."],
  ["The Player queue is empty.", "La fila del Player está vacía."],
  ["Open Player to use Plan B or add a singer.", "Abre el Player para usar Plan B o agregar un cantante."],
...
function interpolateEnglishToSpanish(value) {
  const patterns = [
    ...
    [/^(\d+) active Player request · (.+) already performed$/, "$1 solicitud activa del Player · $2 ya interpretado"],
    [/^(\d+) active Player requests · (.+) already performed$/, "$1 solicitudes activas del Player · $2 ya interpretado"],
    ...
    [/^(\d+)% covered by completed songs and the Player queue$/, "$1% cubierto por canciones completadas y la fila del Player"],
