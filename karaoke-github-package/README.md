...
- una fila agregada directamente en VirtualDJ conserva su procedencia como
  ...
  duración con una solicitud;
- los vínculos inferidos exigen que el cantante físico de VirtualDJ coincida
  con quien hizo la solicitud. Los vínculos incorrectos de versiones anteriores
  se vuelven a validar contra el cantante original;
- un vínculo manual explícito sigue siendo autoritativo al reordenar o bajar la
  coincidencia y solo su identificador físico exacto puede reclamar una pista
  previamente reconocida como externa.

## Incluido desde 4.3.8
...
- las decisiones terminales y retiradas de VirtualDJ se guardan localmente por
  ...
  pueden volver a insertar una canción; solo **Deshacer** revierte el resultado;
- la cola real de VirtualDJ mantiene el vínculo al mover canciones y exige tres
  lecturas ausentes antes de confirmar una retirada;
- Solicitudes y cola real de VirtualDJ aparecen lado a lado; completadas y
  omitidas quedan en una bandeja compacta debajo;
- la página pública estática reintenta fallos transitorios, y la réplica hacia
  Sheets usa una exclusión global entre Workers para resistir ráfagas.

## Incluido desde 4.3.6

- las consultas públicas, del Host y del Bridge ya no ejecutan respaldos de
  Google Sheets; solo las mutaciones confirmadas programan un lote pequeño;
- los respaldos concurrentes se agrupan y los latidos operativos del Bridge no
  generan eventos históricos, evitando límites de CPU del Worker;
- la página pública consulta el estado cada 15 segundos y conserva el último
  estado válido ante un fallo transitorio, sin mostrar falsamente que el enlace
  está inactivo después de enviar una solicitud.

## Incluido desde 4.3.5

- la cola Karaoke se lee de forma ordenada y se valida contra un segundo conteo,
  evitando filas perdidas y desconexiones causadas por ráfagas a Network Control;
- las consultas seguras se reintentan, pero un comando que agrega una canción
  nunca se repite automáticamente si su confirmación está pendiente;
- VirtualDJ recibe nombres humanos como `Alex A` o `Alex B`; los códigos internos
  `G-…` dejan de escribirse y también se ocultan al leer filas antiguas;
- cada fila física de VirtualDJ permanece visible y autoritativa, incluidas las
  pistas externas y copias iguales; el Bridge ya no borra filas por heurística.

## Incluido desde 4.3.4
