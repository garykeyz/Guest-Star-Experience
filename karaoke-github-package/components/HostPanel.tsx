...
export default function HostPanel({ oneTimeCode = "" }: { oneTimeCode?: string }) {
  ...
  return <main className="hostPage">
    <header className="hostTop"><div><p className="hostEyebrow">GUEST STAR EXPERIENCE 4.3.7</p><h1>{user.role==="superhost"?"Superhost Administration":"Host Panel"}</h1><span>{user.displayName} · {user.role}{user.role==="superhost"&&codeVersion?` · Service v${codeVersion}`:""}</span></div><div className="entityLinks"><button onClick={()=>setShowOwnPasswordForm(value=>!value)}><KeyRound/>Change Password</button><button onClick={logout}><LogOut/>Log Out</button></div></header>
